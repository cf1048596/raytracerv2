...
pub fn write_color(pixel_color: &Color) {
    ...
    let r = if pixel_color.x() > 0.0 { pixel_color.x().sqrt() } else { 0.0 };
    let g = if pixel_color.y() > 0.0 { pixel_color.y().sqrt() } else { 0.0 };
    let b = if pixel_color.z() > 0.0 { pixel_color.z().sqrt() } else { 0.0 };
